...
class EmailService {
  ...
  /**
   * Send a generic support email to site admin
   */
  async sendSupportEmail(fromName: string, fromEmail: string, subject: string, message: string): Promise<void> {
    try {
      const admin = process.env.SUPPORT_EMAIL || process.env.SMTP_USER;
      // Use authenticated SMTP user as the envelope 'from' to avoid provider rejections.
      // Put the sender's address in replyTo so admins can reply directly to the user.
  ...
  /**
   * Send password changed notification email (admin changed password)
   */
  async sendPasswordChangedNotification(user: User, newPassword: string): Promise<void> {
    try {
      const mailOptions = {
        from: `"${process.env.APP_NAME}" <${process.env.SMTP_USER}>`,
        to: user.Email,
      ...
      throw new DatabaseError('Failed to send password changed notification');
    }
  }
   ...
   */
  private getVerificationEmailTemplate(user: User, verificationUrl: string): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
   ...
   */
  private getPasswordChangedTemplate(user: User, newPassword: string): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
